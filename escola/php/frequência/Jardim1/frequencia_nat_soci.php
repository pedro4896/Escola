<?php
    ...
    WHERE ensina.codigo_disciplinas = 'J1_NAT_SOC' AND leciona.turma_serie = 'Jardim 1'");
    $cont = $banco->query("SELECT COUNT(matricula) AS quantidade FROM discentes WHERE turma_serie = 'Jardim 1';");
    $discente = $banco->query("SELECT * FROM discentes WHERE turma_serie = 'Jardim 1' AND situacao = 'Deferido' ORDER BY nome ASC;");
    $falta = array();
    $presenca = array();
    $justificativa = array();
    if($cont){
    ...
    for($x = 1; $x<($indice + 1);$x++){
        $frequencia = $banco->query("INSERT INTO `frequencia` (`data_frequencia`, `faltas`, `presenca`, `justificativa`, `codigo_disciplinas`, `matricula_discente`, `cpf_usuario`, `turma_serie`)
                                     VALUES ('$data', $falta[$x], $presenca[$x], '$justificativa[$x]', 'J1_NAT_SOC', '$matricula[$x]', '$cpf', 'Jardim 1');");
        if($frequencia){}
    }   
    
    $aula = $banco->query("INSERT INTO `aula` (`data_aula`, `pauta`, `codigo_disciplinas`, `cpf_usuario`, `turma_serie`)
                           VALUES ('$data', '$pauta', 'J1_NAT_SOC', '$cpf', 'Jardim 1')");
    
    if($aula){
        echo("<script>window.location.href='educacao_fisica.php';</script>");
    }else{
