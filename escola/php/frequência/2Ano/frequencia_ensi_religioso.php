<?php
    ...
    $cont = $banco->query("SELECT COUNT(matricula) AS quantidade FROM discentes WHERE turma_serie = '2 Ano';");
    $discente = $banco->query("SELECT * FROM discentes WHERE turma_serie = '2 Ano' AND situacao = 'Deferido' ORDER BY nome ASC;");
    $falta = array();
    $presenca = array();
    $justificativa = array();
    if($cont){
    ...
    for($x = 1; $x<($indice + 1);$x++){
        $frequencia = $banco->query("INSERT INTO `frequencia` (`data_frequencia`, `faltas`, `presenca`, `justificativa`, `codigo_disciplinas`, `matricula_discente`, `cpf_usuario`, `turma_serie`)
                                     VALUES ('$data', $falta[$x], $presenca[$x], '$justificativa[$x]', '2_ENSI_RELI', '$matricula[$x]', '$cpf', '2 Ano');");
        if($frequencia){}
    }   
    
    $aula = $banco->query("INSERT INTO `aula` (`data_aula`, `pauta`, `codigo_disciplinas`, `cpf_usuario`, `turma_serie`)
                           VALUES ('$data', '$pauta', '2_ENSI_RELI', '$cpf', '2 Ano')");
