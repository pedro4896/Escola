<?php
    ...
    WHERE ensina.codigo_disciplinas = 'J3_NAT_SOC' AND leciona.turma_serie = 'Jardim 3'");
    $cont = $banco->query("SELECT COUNT(matricula) AS quantidade FROM discentes WHERE turma_serie = 'Jardim 3';");
    $discente = $banco->query("SELECT * FROM discentes WHERE turma_serie = 'Jardim 3' AND situacao = 'Deferido' ORDER BY nome ASC;");
    $falta = array();
    $presenca = array();
    $justificativa = array();
    if($cont){
    ...
    for($x = 1; $x<($indice + 1);$x++){
        $frequencia = $banco->query("INSERT INTO `frequencia` (`data_frequencia`, `faltas`, `presenca`, `justificativa`, `codigo_disciplinas`, `matricula_discente`, `cpf_usuario`, `turma_serie`)
                                     VALUES ('$data', $falta[$x], $presenca[$x], '$justificativa[$x]', 'J3_NAT_SOC', '$matricula[$x]', '$cpf', 'Jardim 3');");
        if($frequencia){}
    }   
    
    $aula = $banco->query("INSERT INTO `aula` (`data_aula`, `pauta`, `codigo_disciplinas`, `cpf_usuario`, `turma_serie`)
                           VALUES ('$data', '$pauta', 'J3_NAT_SOC', '$cpf', 'Jardim 3')");
