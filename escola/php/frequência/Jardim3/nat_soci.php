...
<!DOCTYPE html>
            <html>
                <head>
                    ...
                    <title>Frequência Jardim3 - Natureza e Sociedade</title>
                ...
                <body> 
                    ...
                    <div><table>
                        <form action="frequencia_nat_soci.php" method="POST" onsubmit=" return validarTudo()">
                            <tr><th colspan=5>Frequência Jardim 3 - Natureza e Sociedade</th></tr>
                            <tr id=titulo>
                                <?php
                                    ...
                                    WHERE ensina.codigo_disciplinas = 'J3_MAT' AND leciona.turma_serie = 'Jardim 3'");
                                    if($result){
                                    ...
                                    echo("<td>Docente:".@$nome."</td><td>Disciplina: Natureza e Sociedade</td><td>S&eacute;rie: ".@$serie."</td><td colspan=2>Data: <input type=date name=data  id=data required></td>");
                                ?>
                            </tr>
                            <tr><td>Matr&iacute;cula</td><td>Nome</td><td>Presen&ccedil;a</td><td>Faltas</td><td>Justificativa</td><tr>
                            <tbody id=tbody>
                                <?php
                                    $result = $banco->query("SELECT * FROM discentes WHERE turma_serie = 'Jardim 3' AND situacao = 'Deferido' ORDER BY nome ASC;");
                                    if($result){
                                        $cont = 1;
                                        foreach($result as $linha){
                                            echo("<tr>");
